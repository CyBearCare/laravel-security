...
return new class extends Migration
{
    public function up()
    {
        Schema::create('cybear_waf_rules', function (Blueprint $table) {
            ...
            $table->unsignedInteger('version')->default(1)->comment('Monotonic rule version');
            $table->json('scope')->nullable()->comment('Exact method, host, route name, and route URI scope');
            $table->unsignedTinyInteger('rollout_percentage')->default(100)->comment('Deterministic rollout percentage');
            $table->timestamp('expires_at')->nullable()->index()->comment('Rule expiry time');
            $table->string('finding_id', 100)->nullable()->index()->comment('Related scanner finding');
            $table->string('owner', 200)->nullable()->comment('Rule owner');
            $table->text('reason')->nullable()->comment('Reason the rule exists');
