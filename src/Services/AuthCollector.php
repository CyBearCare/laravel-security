...
class AuthCollector extends BaseDataCollector
{
    ...
    protected function collectData(): array
    {
        ...
        if (config('cybear.collectors.auth.collect_guard_config', true)) {
            $data['guards'] = $this->getGuardConfigurations();
            $data['providers'] = $this->getProviderConfigurations();
            $data['passwords'] = $this->getPasswordConfigurations();
        }
        if (config('cybear.collectors.auth.collect_user_stats', false)) {
            $data['user_statistics'] = $this->getUserStatistics();
        }
        if (config('cybear.collectors.auth.collect_session_config', true)) {
            $data['session_config'] = $this->getSessionConfiguration();
        }
